 ...
 *   node scripts/enrich-insights-nf-geo.mjs --cnpj 29736661000161
 *   node scripts/enrich-insights-nf-geo.mjs --cnpj 29736661000161 --cnpj 00000000000191
 *
...
Uso:
  node scripts/enrich-insights-nf-geo.mjs --upload-id <uuid>
  node scripts/enrich-insights-nf-geo.mjs --last-upload
  node scripts/enrich-insights-nf-geo.mjs --cnpj <14 dígitos ou mascarado> [--cnpj ...]
...
async function main() {
  const { uploadId: idArg, lastUpload, dryRun, noNominatim, cnpjs } = parseArgs(process.argv)
  ...
  let unique = []
  /** @type {string} */
  let resolvedUploadId = ''

  if (cnpjs.length > 0) {
    for (const raw of cnpjs) {
      const d = normalizeCnpjDigits(raw)
      if (d.length === 14) unique.push(d)
      else console.warn('Ignorando CNPJ inválido:', raw)
    }
    unique = [...new Set(unique)]
    if (unique.length === 0) {
      console.error('Nenhum CNPJ válido após --cnpj')
      process.exit(1)
    }
    console.log(
      'Modo --cnpj:',
      ...
      'cliente(s)',
      dryRun ? '(dry-run)' : '',
      noNominatim ? '(sem Nominatim)' : ''
    )
  } else {
    let uploadId = idArg
    if (lastUpload || !uploadId) {
      if (!lastUpload && !uploadId) {
        console.error('Informe --upload-id, --last-upload ou --cnpj')
        process.exit(1)
      }
    ...
    unique = [...new Set(nfs.map((r) => r.cnpj_cliente))]
  ...
  console.log(
    cnpjs.length > 0
      ? 'Enriquecimento concluído para o(s) CNPJ informado(s).'
      : `Atualização incremental concluída para upload ${resolvedUploadId}`
  )
}
