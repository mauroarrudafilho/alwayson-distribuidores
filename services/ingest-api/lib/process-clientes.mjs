...
const REQUIRED = ['cnpj', 'razao_social', 'cidade', 'estado']
const ALIASES = {
  cnpj: ['cnpj_cliente'],
  razao_social: ['razao'],
...
export async function processClientes(supabase, { buffer, distribuidorId }) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const cidade = cellStr(pick(row, 'cidade')) || '—'
    const estado = cellStr(pick(row, 'estado')).toUpperCase().slice(0, 2) || '—'
    const codVend = cellStr(pick(row, 'codigo_vendedor'))
    const nomeVend = cellStr(pick(row, 'nome_vendedor'))

    if (!cnpj || !razao) {
      warnings.push(`Linha ${i + 2}: cnpj/razão inválidos`)
      continue
    }

    const vendedorId = await ensureVendedor(codVend, nomeVend)
