...
- **Falha fechada:** `NULL IN (...)` é NULL, então linha sem `fornecedor_tenant_id` fica invisível a não-admin. Por isso a migration `049` põe um trigger que carimba o fornecedor da meta quando o distribuidor tem só um — com dois ou mais, deixa NULL de propósito e a aplicação precisa escolher. Ao criar tabela nova com os dois eixos, lembre do carimbo, senão a linha nasce invisível.
